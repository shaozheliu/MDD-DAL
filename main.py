...
from models.model_factory import prepare_training, train_model_with_domain, test_evaluate, test_evaluate_2b
...
# 指定文件夹路径
folder_path = '/home/alk/Data/MDD/'
domain_num = 3
# 调用函数处理文件
subject_lists = process_files(folder_path)
...
# 打印日志设定
outputfile = open(f'./logs/exp_{model_name}_dropout:{dropout}_attdim:{d_model}_encoder_num:{n_layers}_lr:{lr}_domainnum:{domain_num}.txt', 'w')
...
for i, (train_idx, test_idx) in enumerate(loo.split(subject_lists)):
    ...
    GetData_train = DataProcess(data_path, data_files, train_subjects, True, domain_num)
    X_train, y_train, y_train_domain = GetData_train.data, GetData_train.label, GetData_train.domain_label
    GetData_test = DataProcess(data_path, data_files, test_subjects,True, 1)
